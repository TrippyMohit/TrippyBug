...
import { CarIcon, ChevronDownIcon, ChevronUpIcon, FAQsIcon, FlightIcon, HotelIcon } from "../icons";
...
export default function Profile() {
  ...
  return (
    <>
      <div className="">
        <Image alt="" objectFit="contain" layout="fill" src="/assets/images/faq-bg.svg" />
      </div>
      <div className=" container mt-20 flex flex-col gap-12 ">
        <div className='flex flex-row  '>
          <div className=' font-bold flex text-4xl text-gray-500 w-3/4'>Frequently Asked Questions</div>
    ...
    <div className='flex flex-col w-full gap-4 '>
      <div className='flex bg-gray-50  p-2 gap-5 rounded-md'>
        <div className=' flex bg-orange-100 justify-center items-center p-3 w-14 h-14 text-orange-400 rounded-full'>
          <div className=" w-6 text-orange-400" >{FAQsIcon}
          </div>
        </div>
        <div className='font-medium text-2xl text-gray-600 flex items-center'>General </div>
      </div>
      {generalFaq.map((faq, i) => {
        return (
          <div key={i} className="rounded-md flex  flex-col gap-4" >
            <div className="p-4 text-xl relative font-medium rounded-md bg-gray-50 flex justify-between cursor-pointer" onClick={() => { expand == i ? setExpand(null) : setExpand(i) }}>
            ...
            <div className={classNames("p-4", {
              "flex transition-all ease-in-out duration-1000": expand == i, "hidden": expand != i,
            })}>
              {faq.answer}
            </div>
          </div>
...
const CarsFaq = () => {
  ...
  return (
    <div className='flex flex-col w-full gap-4 '>
      <div className='flex bg-gray-50 p-2 gap-5 rounded-md'>
        <div className=' flex bg-orange-100 justify-center items-center p-3 w-14 h-14 text-orange-400 rounded-full'>
          <div className=" w-6 text-orange-400">{CarIcon}
          </div>
        </div>
        <div className='font-medium text-2xl text-gray-600 flex items-center'>Cars </div>
      </div>
      {carsFaq.map((faq, i) => {
...
const HotelsFaq = () => {
  ...
  return (
    <div className='flex flex-col w-full gap-4 '>
      <div className='flex bg-gray-50  p-2 gap-5 rounded-md'>
        <div className=' flex bg-orange-100 justify-center items-center p-3 w-14 h-14 text-orange-400 rounded-full'>
          <div className=" w-6 text-orange-400">{HotelIcon}
          </div>
        </div>
        <div className='font-medium text-2xl text-gray-600 flex items-center'>Hotels </div>
      </div>
      {hotelsFaq.map((faq, i) => {
        return <div key={i} className="rounded-md flex  flex-col gap-4" >
          <div className="p-4 text-xl relative font-medium rounded-md bg-gray-50 flex justify-between cursor-pointer" onClick={() => { expand == i ? setExpand(null) : setExpand(i) }}>
...
const FlightsFaq = () => {
  ...
  return (
    <div className='flex flex-col w-full gap-4 '>
      <div className='flex bg-gray-50  p-2 gap-5 rounded-md'>
        <div className=' flex bg-orange-100 justify-center items-center p-3 w-14 h-14 text-orange-400 rounded-full'>
          <div className=" w-6 text-orange-400">{FlightIcon}
          </div>
        </div>
        <div className='font-medium text-2xl text-gray-600 flex items-center'>Flights </div>
      </div>
