...
const AirlinesBlogs = ({ airlinesBlogs }) => {
  ...
  const AirlinesBlogsRef = createRef<Slider>();

  const previousSlide = () => {
    AirlinesBlogsRef?.current.slickPrev();
  };

  const nextSlide = () => {
    AirlinesBlogsRef?.current?.slickNext();
  };
      ...
      <div className="relative flex container w-full mx-auto pt-16 pb-8 gap-10 lg:flex-row-reverse items-center flex-col ">
        <div className="flex flex-1 justify-between w-full lg:w-9/12">
          <div className="relative w-full">
            <Slider {...trendingCarouselSettings} ref={AirlinesBlogsRef}>
              {airlinesBlogs?.map((post, index) => (
                <TrendingBlogCard
                  key={index}
                  date={
  ...
  return (
    <div
      className={classNames(
        "flex flex-col gap-6 items-start justify-between mx-2 mb-8",
        { "lg:pt-0": index % 2 == 1 }
      )}
    >
      <Link href={link}>
