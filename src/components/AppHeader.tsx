import { CarIcon, FlightIcon, MenuIcon, HotelIcon, AboutUsIcon, GalleryIcon, BlogsIcon, CommunityIcon, ContactUsIcon, FAQsIcon, LoginIcon, UserIcon, UserOutlineIcon, GearIcon, LogoutIcon } from "../icons";
...
export const AppHeader = () => {
  ...
  return (
    <>
      ...
      <nav className={classNames("z-50 sticky flex justify-between lg:py-4 gap-20 lg:px-28 items-center h-24 overflow-visible", {
        'lg:bg-transparent mb-0 lg:-mb-24  top-0 bg-white lg:relative z-50': route.pathname == "/",
        ...
        >
          ...
          >
            <div className="bg-white min-h-screen w-8/12 flex flex-col align-top overflow-hidden justify-start p-6" >
              ...
              {
                menu.map((menuName) =>
                  <div className="text-gray-400 font-medium text-xs mt-2" key={menuName.label}>
        ...
        <div className="w-3/12 flex justify-end gap-4 leading-7">
          {session ?
            <>
              ...
              <button className="flex gap-4 items-center relative pr-10" onClick={() => setIsAvatarMenuOpen(!isAvatarMenuOpen)}>
                <div className="font-semibold">{session?.user?.name?.split(' ')?.[0]}</div>
